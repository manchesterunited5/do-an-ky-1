...
@section('content')
        ...
        <div class="row">
            <div class="col-lg-4">
                <div class="shadow-lg p-3 mb-5 bg-body rounded">
                    <form action="{{route('admin-add-rating')}}" method="post" onsubmit="return validate()">
                        @csrf
                ...
                <table class="table table-bordered">
                    ...
                    @foreach($ratings as $rating)
                        <tr>
                            ...
                            <td>{{$rating->name}}</td>
                            <td>
                                <span class="badge badge-{{$rating->status==1?'success':'danger'}}">{{$rating->status==1?'active':'hide'}}</span>
                            </td>
                            <td>
                                <div class="btn-group" role="group">
                                    <div>
                                        <button type="button" class="btn btn-warning btn-sm mr-1"
                                                onclick="updateRating({{$rating->id}},'{{$rating->name}}','{{$rating->status}}')">
                                            <i class="fas fa-edit pr-1"></i>Sửa
                                        </button>
                                    </div>
                                    <form action="{{route('admin-delete-rating', $rating->id)}}" method="post">
                                        @method('delete')
                                        @csrf
    ...
    <script>
        ...
        function updateRating(id, name, status) {
            let hidden = $('#hidden');
            let ratingName = $('#name');
